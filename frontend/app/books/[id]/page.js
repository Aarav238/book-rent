...
export default function BookDetails({ params }) {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        ...
        if (data.owner) {
          const ownerResponse = await fetch(`http://localhost:5000/api/auth/profile/${data.owner._id}`);
          if (ownerResponse.ok) {
  ...
  const handleRequestBook = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/book/${id}/request`, {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="max-w-3xl mx-auto py-8 px-4">
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-500">The book you are looking for does not exist or may have been removed.</p>
...
/>
          ...
          <div className="md:w-2/3 p-6">
            ...
            {owner && (
              <div className="mb-6">
                ...
                <p className="text-gray-600">{owner.user.name || 'Book Owner'}</p>
              </div>
            )}
              ...
              <div className="mt-6">
                {requestSent ? (
                  <div className="bg-green-100 text-green-800 p-4 rounded-lg">
                    ...
                    <p className="mt-2">The owner has been notified of your interest. You can use the contact information above to coordinate the exchange.</p>
                  </div>
                ) : (
                  <button
            ...
            {(!user || (user.role === 'Seeker' && book.status !== 'Available')) && (
              <div className="mt-6 p-4 bg-gray-100 rounded-lg">
                {book.status !== 'Available' ? (
                  <p className="text-gray-700">This book is currently borrowed by someone else. Check back later or browse other available books.</p>
                ) : (
                  <p className="text-gray-700">Sign in as a Seeker to request this book.</p>
                )}
              </div>
            )}
          </div>
      ...
      <div className="mt-8 text-center">
        <h3 className="text-xl font-semibold text-gray-700 mb-4">Similar Books</h3>
        <p className="text-gray-500">Looking for related books? Check out our recommendations in the same genre.</p>
        <Link 
